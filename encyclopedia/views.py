...
def entry_page(request, title):
    ...
    html_entry_contents = markdown_to_html(
        entry_contents) if entry_contents else None
...
def markdown_to_html(markdown_string):
    ...
    heading_matcher = re.compile(
        r'^(?P<hash_tags>#{1,6})\s*(?P<heading_title>.*)$', re.MULTILINE)
    ...
    bold_substituted = bold_matcher.sub(
        r"<b>\g<bolded_content></b>", heading_substituted)

    list_outside_matcher = re.compile(
        r"^([-*])\s+.*?(?=\n\n|\n((?!\1)[-*])\s|\Z)", re.MULTILINE | re.DOTALL)
    list_inside_matcher = re.compile(
    ...
    link_matcher = re.compile(
        r"\[(?P<text>((?!\n\n).)*?)\]\((?P<link>((?!\n\n).)*?)\)", re.DOTALL)
    link_substituted = link_matcher.sub(
        rf'<a href="\g<link>">\g<text></a>', list_substituted)

    paragraph_matcher = re.compile(rf"^(?!{unique_marker}|\n|\Z)(?P<paragraph_text>.*?)(?=(\n\n)|{unique_marker}|\Z)",
                                   re.MULTILINE | re.DOTALL)
    paragraph_substituted = paragraph_matcher.sub(
        r"<p>\g<paragraph_text></p>", link_substituted)
